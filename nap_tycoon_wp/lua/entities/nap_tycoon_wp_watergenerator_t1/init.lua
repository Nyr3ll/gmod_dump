...
function ENT:Initialize()
    ...
    self:SetCollisionGroup(NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.HasCollision)

    local maxHealth = NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.Health
    self:SetNWInt("Health", maxHealth)
    self:SetNWInt("MaxHealth", maxHealth)
    self:SetHealth(maxHealth)

    self:SetNWFloat("WatermoneyProduction", NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.WaterMoneyOutput)
    self:SetNWFloat("ProcessingTime", NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.ProcessingTime)
...
function ENT:OnTakeDamage(dmginfo)
    if NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.HealthModule  then
        ...
        if currentHealth <= 0 then
            ...
            if NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.BeRewardToDestroyGenerator then
                if attacker == owner and NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT1.BeRewardToDestroyOwnedGenerator then
