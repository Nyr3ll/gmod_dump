...
local function CreateStyledToggle(parent, x, y, string)
    local toggle = vgui.Create("DButton", parent)
    toggle:SetSize(60*resolutionMultiplierW, 30*resolutionMultiplierW)
    toggle:SetText("")
    toggle:SetPos(x, y)
    toggle.On = LocalPlayer():GetNWBool("NAP_DEVTOOL_"..string)

    toggle.Paint = function(self, w, h)
        draw.RoundedBox(15, 0, 0, w, h, self.On and Color(0, 120, 215) or Color(230, 230, 230))

        local padding = 3
        local circleSize = h - padding * 2
        local x = self.On and (w - circleSize - padding) or padding

        draw.RoundedBox(circleSize / 2, x, padding, circleSize, circleSize, Color(255, 255, 255))
    end
    ...
    toggle.DoClick = function(self)
        ...
        net.Start("NAP_DEVTOOL_ViewsTogglerUpdater")
            net.WritePlayer(LocalPlayer())
            net.WriteString(string)
            net.WriteBool(self.On)
        net.SendToServer()
    end
...
-- ///////////////////////////////////////////////////////////////////////
-- // Function to generate a "DButton" really compact in final UI code //
-- /////////////////////////////////////////////////////////////////////
local function AddDButton(parent, w, h, x, y, color, text, tooltip)
    local btn = vgui.Create("DButton", parent)
    btn:SetSize(w, h)
    btn:SetPos(x, y)
    btn:SetColor(color)
    btn:SetText(text)
    btn:SetFont("NAP_MONEY_MARKET_Font_4")
    btn:SetTooltip(tooltip)
    return btn
end

-- //////////////////////////////////////////////////////////////////////
-- // Function to generate a "DLabel" really compact in final UI code //
-- ////////////////////////////////////////////////////////////////////
...
function DEVTOOL_Page_Two()
    ...
    local labelX = AddDLabel(frame, "X :", hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", labelPosX, labelPosY)
    ...
    local labelY = AddDLabel(frame, "Y :", hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", labelPosX*3.5, labelPosY)
    ...
    local labelZ = AddDLabel(frame, "Z :", hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", labelPosX*6, labelPosY)
    ...
    local destinationsListLabel = AddDLabel(frame, language["teleporter_destination_list"], hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", frameWide*0.1, frameTall*0.325)
    ...
    local labelCoords = AddDLabel(frame, "", hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", 0, 0)
    ...
    local teleporterToPlayer = AddDLabel(frame, language["teleporter_player_destination"], hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", frameWide*0.1, frameTall*0.5)
...
function DEVTOOL_Page_Three()
    ...
    local coords_text = AddDLabel(frame, language["admin_menu_views_coords_text"], hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", frameWide*0.2, frameTall*0.1)
    local coords_toggled = CreateStyledToggle(frame, frameWide*0.6, frameTall*0.1, "Coords")

    local trace_text = AddDLabel(frame, language["admin_menu_views_tracer_text"], hidden_color, white_color, "DEVTOOL_FONT_MainPage_Button", frameWide*0.2, frameTall*0.2)
    local trace_toggled = CreateStyledToggle(frame, frameWide*0.6, frameTall*0.2, "Traces")
...
function DEVTOOL_OpenBurgerMenu(parentFrame)
    ...
    CreateBurgerButton(language["admin_menu_views"], 150, function()
        parentFrame:Close()
        DEVTOOL_Page_Three()
    end)
