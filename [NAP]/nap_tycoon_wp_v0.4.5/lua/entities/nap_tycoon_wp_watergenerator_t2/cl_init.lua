...
function ENT:Draw()
    ...
    if distance <= showTextDistance then
        ...
        cam.Start3D2D(pos, ang, 0.2)
            ...
            local WaterGen_T1_RoundedBox = NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.RoundedBoxColor

            -- Title
            draw.RoundedBox( 16, -100, -300, 200, 40, WaterGen_T1_RoundedBox)
            draw.SimpleText(self.PrintName, "Trebuchet24", 0, -280, NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.TitleTextColor, 1, 1)
            ...
            draw.SimpleText(string.format("%.1f", self:GetNWFloat("Health")).." HP", "Trebuchet24", 0, -230, NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.HealthBarTextColor, 1, 1)

            -- Producing rate
            local quantityProduced = NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.WaterMoneyOutput
            local timeToProduced = NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.ProcessingTime
            draw.RoundedBox( 16, -100, -200, 200, 40, WaterGen_T1_RoundedBox)
            draw.SimpleText(quantityProduced/timeToProduced.." WM / Sec", "Trebuchet24", 0, -180, NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.TitleTextColor, 1, 1)        

            -- Processing bar
            draw.RoundedBox( 16, -100, -150, 200, 40, WaterGen_T1_RoundedBox)
            draw.RoundedBox( 16, -97.5, -145, 195-(((roundedBoxWdth/(self:GetNWFloat("ProcessingTime")+1))*self:GetNWFloat("ProcessingTimeRemaining"))), 30, NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.WorkingRoundedBoxColor)
            draw.SimpleText(self:GetNWFloat("ProcessingTimeRemaining").." s", "Trebuchet24", 0, -130, NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.WorkingTextColor, 1, 1)       
