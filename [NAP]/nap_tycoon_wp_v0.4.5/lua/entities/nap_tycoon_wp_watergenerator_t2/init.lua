...
function ENT:Initialize()
    ...
    self:SetColor(Color(0, 242, 255, 255))
    self:PhysicsInit(SOLID_VPHYSICS)
    self:SetMoveType(MOVETYPE_VPHYSICS)
    self:SetSolid(SOLID_VPHYSICS)
    self:SetCollisionGroup(NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.HasCollision)

    local maxHealth = NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.Health
    self:SetNWInt("Health", maxHealth)
    self:SetNWInt("MaxHealth", maxHealth)
    self:SetHealth(maxHealth)

    self:SetNWFloat("WatermoneyProduction", NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.WaterMoneyOutput)
    self:SetNWFloat("ProcessingTime", NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.ProcessingTime)
...
function ENT:OnTakeDamage(dmginfo)
    if NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.HealthModule  then
        ...
        if currentHealth <= 0 then
            ...
            if NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.BeRewardToDestroyGenerator then
                if attacker == owner and NAP_TYCOON_WATERPRODUCTION.Config.Entities.WaterGeneratorT2.BeRewardToDestroyOwnedGenerator then
